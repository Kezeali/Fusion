...
FusionMessage *MessageBuilder::BuildMessage(const FusionEngine::ProjectileState &input, ObjectID SystemAddress)
{
	RakNet::BitStream out_stream;
...
FusionMessage *MessageBuilder::BuildMessage(const ShipInput &input, ObjectID SystemAddress)
{
	RakNet::BitStream out_stream;
...
FusionMessage *MessageBuilder::BuildMessage(Packet *packet, ObjectID SystemAddress)
{
	FusionMessage *m;
	...
	{
		m = new FusionMessage(CID_SYSTEM, MTID_NEWPLAYER, SystemAddress, data, packet->length);
	}
	...
	{
		m = new FusionMessage(CID_FILESYS, MTID_STARTTRANSFER, SystemAddress, data, packet->length);
	}
	...
	{
		m = new FusionMessage(CID_GAME, MTID_PROJECTILEFRAME, SystemAddress, data, packet->length);
	}
	...
	{
		m = new FusionMessage(CID_CHAT, MTID_CHALL, SystemAddress, data, packet->length);
	}
	// To team chat data
	else if (packetid == MTID_CHTEAM)
	{
		m = new FusionMessage(CID_CHAT, MTID_CHTEAM, SystemAddress, data, packet->length);
	}
	// To a specific player chat data
	else if (packetid == MTID_CHONE)
	{
		m = new FusionMessage(CID_CHAT, MTID_CHONE, SystemAddress, data, packet->length);
	}
