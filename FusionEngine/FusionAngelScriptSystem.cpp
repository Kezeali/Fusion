		...
		auto execute_scripts = [&](const tbb::blocked_range<size_t>& range)
		{
			...
			for (size_t ri = range.begin(), rend = range.end(); ri != rend; ++ri)
			{
				...
				if (script->m_ScriptObject)
				{
					...
					{
						auto& ctx = it->first;
						if (!it->second.condition || it->second.condition()) // If this coroutine has no condition, or it's condition is satisfied
						{
							int r = ctx->Execute();
					...
					for (auto it = script->m_ActiveCoroutinesWithConditions.begin(); it != script->m_ActiveCoroutinesWithConditions.end(); )
					{
						script->m_ActiveCoroutines.push_back(std::make_pair(it->second.ctx, it->second));
					}
